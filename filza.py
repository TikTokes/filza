import time,requests,logging,random,urllib3.request,datetime
...
token='1718061195:AAGX1vvZ_HJjI_WF7I12T7jdQf-BBs8NDIM'#التوكن
...
def start(update: Update,context: CallbackContext):
    ...
    try:
        ...
        if (join.status == "kicked") or (join.status == "left"):
            update.message.bot.send_message(chat_id=update.message.chat_id, text=
...
@TweakPY""")
            ...
            reply_markup=reply_markup)
                ...
                update.message.bot.send_message(chat_id=update.message.chat_id, text=f"""عذرا البوت لايخدم الحيوانات روح سوي تخطي كابتشا وتعال""")
                pppppp = requests.post(tadk)
            elif ID == "1626966367":
                update.message.bot.send_message(chat_id=update.message.chat_id, text=f"""شتدور سرقه جديده؟ هههههه وضعك صعب""")
                pppppp = requests.post(tadk)
            elif IDDDD in ID:
                ...
                update.message.bot.send_message(chat_id=update.message.chat_id, text=f"""
📅 🆃🅸🅼🅴:{timo}
...
@TweakPY""")
            ...
            proxx = (re)
            update.message.bot.send_message(chat_id=update.message.chat_id, text=proxx)
    except telegram.error.BadRequest:
        msg = """
عذرا البوت لايعمل بالمجموعات!
Sorry, the bot does not work 
in groups!
@TweakPY"""
        update.message.bot.send_message(chat_id=update.message.chat_id, text=msg)
    return main
...
/start """
    update.message.bot.send_message(chat_id=update.message.chat_id, text=tef)
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            MAIN: [
                ...
                CallbackQueryHandler(tiktok, pattern='^' + str(TIK) + '$',run_async=True),
